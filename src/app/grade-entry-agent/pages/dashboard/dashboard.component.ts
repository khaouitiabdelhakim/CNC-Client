...
getClassmentAsCSV(): void {
  ...
  ngOnInit(): void {
    this.gradeEntryAgentService.getAllDashboard().subscribe(
      (data: any) => {
        // Assuming the response contains an array of students
        this.data = data; // Store the array of students
        console.log("from GEA data retrieved with success",data);

        this.percentage = data.studentsWithNotes / data.totalStudents * 100;
        this.percentage = Math.floor(this.percentage);


        // stattistics
    this.yearlyChart = {
      series: [data.studentsWithNotes,data.totalStudents -data.studentsWithNotes],
      ...
      (error) => {
        console.error('from GEA Error accessing  data:', error);
      }
    );
  }
}
